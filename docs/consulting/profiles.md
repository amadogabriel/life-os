# Profile copy — ready to paste on clearance day

**Publish order (least babysitting first):** vetted networks bring clients to
you, so they go live first; marketplaces second; LinkedIn flip last.

## 1. Vetted network applications (Toptal, Lemon.io, Arc.dev)

Apply to all three the same week — acceptance is selective (~1–3%), decisions
are independent, and one acceptance replaces all other channels. Budget ~3
evenings total for screens + a live coding round each. Use the positioning
...
**Screen prep note:** their live rounds are LeetCode-medium + system design;
the measure-theory brain is overkill — do one warm-up evening, no more.
...
>
> Recent public work: Life OS (github.com/amadogabriel/life-os), a React 19 +
> TypeScript + Supabase system with Playwright e2e and ADR-documented
> architecture. Currently completing an MS in Financial Engineering, so
...
- One launch post: what I ship, one slot, link to repo. No follow-up content
  treadmill required — the post is a signpost, not a channel.

## 4. Portfolio page

One static page (agent-built when unblocked): the one-pager + 3 case blurbs
(Life OS; sanitized "document verification tooling at a central bank"; thesis
parallel-computing work) + "one retainer slot" + contact mailto. Hosted as a
GitHub Pages page from a separate public repo — not inside life-os.
