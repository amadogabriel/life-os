    ...
    saveBucket: (bucket, position) =>
      mutate((d) => ({
        ...
        buckets: bucket.id
          ? d.buckets.map((bk) =>
              bk.id === bucket.id
                ? { ...bk, name: bucket.name, cat: bucket.cat, color: bucket.color, counted: bucket.counted, tasks: bucket.tasks.map((t, i) => ({ id: nid(), name: t.name, deep: t.deep, position: i, habitId: t.habitId ?? null, projectId: t.projectId ?? null, sprintId: t.sprintId ?? null })) }
                : bk,
            )
          : [
              ...d.buckets,
              { id: nid(), name: bucket.name, cat: bucket.cat, position, color: bucket.color, counted: bucket.counted, tasks: bucket.tasks.map((t, i) => ({ id: nid(), name: t.name, deep: t.deep, position: i, habitId: t.habitId ?? null, projectId: t.projectId ?? null, sprintId: t.sprintId ?? null })) },
            ],
      })),
    deleteBucket: (id) =>
      mutate((d) => ({
